 ...
 *
 * Inline iframe wiring still runs when `interceptLinks` is false. Link interception opens
 * Apollos URLs in a modal; checkout can redirect to Pushpay inside that modal, which Pushpay
 * blocks (`X-Frame-Options: SAMEORIGIN`). Default is `interceptLinks: false` so Apollos links
 * behave as normal navigation; pass `interceptLinks: true` only when your giving flow is
 * embed-compatible (see Apollos donation embed docs — “Controlling Link Behavior”).
 */
 ...
 * Loads the widget script once and queues `apollos("init", …)` for optional link interception
 * and inline iframe wiring.
 */
export function installApollosWebEmbed(options = {}) {
  ...
  window.apollos('init', {
    ...
    autoOpen: false,
    navigationModalFullscreen: options.navigationModalFullscreen === true,
  });
}
