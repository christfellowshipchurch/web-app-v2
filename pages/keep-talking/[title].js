...
export default function KeepTalkingContent() {
  const router = useRouter();
  const { title } = router.query;

  const options = {
    variables: {
      pathname: `keep-talking/${title}`,
    },
  };
...
export async function getStaticPaths() {
  ...
  return {
    paths: titles.map(title => `keep-talking/${title}`),
...
export async function getStaticProps({ params }) {
  ...
  await apolloClient.query({
    ...
    variables: { pathname: `keep-talking/${params.title}` },
  });
